...
function uploadToGitHub(filename, buffer) {
    return new Promise((resolve, reject) => {
        ...
        const repo = process.env.GITHUB_DATA_REPO || process.env.GITHUB_REPO; // Allow separate data repo
        const branch = process.env.GITHUB_BRANCH || 'main';

        if (!token || !repo) {
            return reject(new Error('Missing GITHUB_TOKEN or GITHUB_REPO/GITHUB_DATA_REPO'));
        }

        // If using a separate data repo, we don't need 'public/' prefix usually, but let's keep it consistent or make it root
        // For simplicity, let's keep path but allow repo switch.
        // If it's a dedicated data repo, maybe root is better? 
        // Let's stick to same path structure for compatibility.
...
function getJson(filename) {
    return new Promise((resolve, reject) => {
        const token = process.env.GITHUB_TOKEN;
        const repo = process.env.GITHUB_DATA_REPO || process.env.GITHUB_REPO;
        const branch = process.env.GITHUB_BRANCH || 'main';
